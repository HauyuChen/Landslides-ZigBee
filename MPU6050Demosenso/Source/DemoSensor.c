...
static void sendReport(uint8 *pkt);
static int8 readTemp(void);
static uint8 readVoltage(void);
bool filter(double new_value);
...
void zb_HandleOsalEvent( uint16 event )
{
  ...
  if ( event & MY_REPORT_EVT )
  {
    ...
    {
      //sendReport();
      //osal_start_timerEx( sapi_TaskID, MY_REPORT_EVT, myReportPeriod );//設置每2秒發送数据*******************************
    }
  }
...
static void sendReport(uint8 *pkt)
{
  ...
  int16 temp;
        ...
        pData[sign]=0x01;//标志位**************************************************************************************************
        pData[sign+1]=0x00;//********************暂时没用到的数据位，后期可扩展***************************************************
        temp=GetData(ACCEL_XOUT_H)-ACCEL_XOUT_H_offset;
        pData[SENSOR_ACCEL_XOUT_H_OFFSET]=  HI_UINT16(temp);
        pData[SENSOR_ACCEL_XOUT_H_OFFSET+1]=  LO_UINT16(temp);
        lcd_printf(dis,temp);
        HalLcdWriteString( dis, HAL_LCD_LINE_2 );
        
        temp=GetData(ACCEL_YOUT_H)-ACCEL_YOUT_H_offset;
        pData[SENSOR_ACCEL_YOUT_H_OFFSET]=  HI_UINT16(temp);
        pData[SENSOR_ACCEL_YOUT_H_OFFSET+1] =  LO_UINT16(temp);
                lcd_printf(dis,temp);
        HalLcdWriteString( dis, HAL_LCD_LINE_3 );

        temp=GetData(ACCEL_ZOUT_H)-ACCEL_ZOUT_H_offset;
        pData[SENSOR_ACCEL_ZOUT_H_OFFSET]=  HI_UINT16(temp);
        pData[SENSOR_ACCEL_ZOUT_H_OFFSET+1] =  LO_UINT16(temp);
                lcd_printf(dis,temp);
        HalLcdWriteString( dis, HAL_LCD_LINE_4 );

        temp=GetData(GYRO_XOUT_H)-GYRO_XOUT_H_offset;
        pData[SENSOR_GYRO_XOUT_H_OFFSET]=  HI_UINT16(temp);
        pData[SENSOR_GYRO_XOUT_H_OFFSET+1] =  LO_UINT16(temp);

        temp=GetData(GYRO_YOUT_H)-GYRO_YOUT_H_offset;
        pData[SENSOR_GYRO_YOUT_H_OFFSET]=  HI_UINT16(temp);
        pData[SENSOR_GYRO_YOUT_H_OFFSET+1] =  LO_UINT16(temp);

        temp=GetData(GYRO_ZOUT_H)-GYRO_ZOUT_H_offset;
        pData[SENSOR_GYRO_ZOUT_H_OFFSET]=  HI_UINT16(temp);
        pData[SENSOR_GYRO_ZOUT_H_OFFSET+1] =  LO_UINT16(temp);
  ...
  //等待修改**********************************************
        ...
        int ACCEL_YOUT;
        int ACCEL_ZOUT;
        ACCEL_XOUT = (pData[SENSOR_ACCEL_XOUT_H_OFFSET] << 8) +pData[SENSOR_ACCEL_XOUT_H_OFFSET+1];
        ACCEL_YOUT = (pData[SENSOR_ACCEL_YOUT_H_OFFSET] << 8) +pData[SENSOR_ACCEL_YOUT_H_OFFSET+1];
        ACCEL_ZOUT = (pData[SENSOR_ACCEL_ZOUT_H_OFFSET] << 8) +pData[SENSOR_ACCEL_ZOUT_H_OFFSET+1];
        if (ACCEL_XOUT >= 32768)
         {
             ACCEL_XOUT -= 65536;
         }
         if (ACCEL_YOUT >= 32768)
          ...
          double ACCEL_X = ACCEL_XOUT / 16384.0 * 9.8;
          double ACCEL_Y = ACCEL_YOUT / 16384.0 * 9.8;
          double ACCEL_Z = ACCEL_ZOUT / 16384.0 * 9.8;
          double ANGLEZ = atan(sqrt(ACCEL_Y * ACCEL_Y + ACCEL_X * ACCEL_X) / ACCEL_Z) * 180 / 3.1415926535;
...
static uint8 readVoltage(void)
{
  ...
  return 0;
    ...
    /*  
    测量山体的稳定坡度，
    限幅滤波，A值可根据实际情况调整，过滤不正常的数据
    value为有效值，new_value为当前采样值  
    滤波程序返回有效的实际值  
    */

    /*
    传入参数：new_value即倾斜角传感器最新的数据（uint8）
    返回参数：true或false(boolean)
    */

    double a=0.8; //定义最大差值
    double Fvalue;//记录上两次的数据，每次更新
    double Svalue;//记录上一次的数据，每次更新
    ...
    bool filter(double new_value)
    {
          ...
          {
            Fvalue=Svalue;//更新数据
            Svalue=new_value;//更新数据
            return true; 
          }
          else
          {
            if ( ( new_value - Svalue > a ) || ( Svalue - new_value > a )) //判断跟上一次的差值
            {
                if(( new_value - Fvalue > a ) || ( Fvalue - new_value > a ))//假设受到干扰，再与上上次的数据做比较
                    {
                        Fvalue=Svalue;    //更新数据
                        Svalue=new_value; //更新数据
                        return false;
